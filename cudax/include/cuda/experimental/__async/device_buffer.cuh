...
private:
  ...
  {
    return _CUDA_VSTD::span<_Type>{self.get(), self.size()};
  }

  friend auto __cudax_launch_transform(stream_ref stream, const device_buffer& self) noexcept
  {
    return _CUDA_VSTD::span<const _Type>{self.get(), self.size()};
  }

  template <class _Deleter>
  device_buffer(_Type* __ptr, size_t __count, stream_ref __stream, _Deleter __fn)
      : device_buffer::unique_ptr(__ptr, device_buffer_deleter(_CUDA_VSTD::move(__fn), __stream))
      , __count_(__count)
  {}
