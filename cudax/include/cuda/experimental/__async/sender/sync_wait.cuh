...
public:
  ...
  _CUDAX_HOST_API auto operator()(_Sndr&& __sndr) const
  {
    using __dom_t = early_domain_of_t<_Sndr>;
    return __dom_t::template __apply<sync_wait_t>(static_cast<_Sndr&&>(__sndr));
  }

  template <class _Sndr, class _Env>
  _CUDAX_HOST_API auto operator()(_Sndr&& __sndr, _Env&& __env) const
  {
    using __dom_t = late_domain_of_t<_Sndr, _Env>;
    return __dom_t::template __apply<sync_wait_t>(static_cast<_Sndr&&>(__sndr), static_cast<_Env&&>(__env));
  }
};
