...
_CUDAX_API auto operator*(__undefined<_Partitioned>&, _Tag (*)(_Args...))
  -> _CUDA_VSTD::__call_result_t<__partitioned_fold_fn<_Tag>, _Partitioned&, __ts<_Args...>&>;
...
struct _CCCL_TYPE_VISIBILITY_DEFAULT __dependent_sender_error : dependent_sender_error
{
  ...
  _CUDAX_API _CUDAX_CONSTEVAL __dependent_sender_error operator+();

  template <class _Ty>
  _CUDAX_API _CUDAX_CONSTEVAL __dependent_sender_error &operator,(_Ty&);

  template <class... _What>
  _CUDAX_API _CUDAX_CONSTEVAL _ERROR<_What...>&operator,(_ERROR<_What...>&);
};
...
_CUDAX_TRIVIAL_API _CUDAX_CONSTEVAL auto __checked_complsigs()
{
  _CUDAX_LET_COMPLETIONS(auto(__cs) = _Completions())
  {
    ...
    {
      return invalid_completion_signature<_CHECKED(_Completions)>();
    }
  }
}
...
_CUDAX_TRIVIAL_API _CUDAX_CONSTEVAL auto __get_completion_signatures_helper()
{
  ...
  {
    return invalid_completion_signature<NO_COMPLETIONS(_Sndr, _Env...)>();
  }
}
...
struct __partitioned_fold_fn<set_value_t>
{
  ...
  _CUDAX_API auto operator()(__partitioned_completions<__ts<_ValueTuples...>, _Errors, _HasStopped>&, _Values&) const
...
struct __partitioned_fold_fn<set_error_t>
{
  ...
  _CUDAX_API auto operator()(__partitioned_completions<_Values, __ts<_Errors...>, _HasStopped>&, __ts<_Error>&) const
