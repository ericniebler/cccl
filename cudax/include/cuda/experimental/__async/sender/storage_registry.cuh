...
{
class __storage_registry_context;
...
struct __storage_registry
{
  ...
  _CUDAX_API auto __write_at(size_t __token, _Ty __value) -> _Ty&
  {
    ...
    auto* __ptr = ::new (__buffer_ + (__descriptors_[__token].__offset - __bytes)) _Ty(_CUDA_VSTD::move(__value));
    if constexpr (!::std::is_trivially_destructible_v<_Ty>)
    {
      ...
      __descriptors_[__token].__destroy = __dtor;
    }
    return *__ptr;
  }

  template <class _Fn, class... _Args>
  _CUDAX_API auto __write_at_from(size_t __token, _Fn fn, _Args&&... args) -> decltype(auto)
  {
    using _Ty              = __decay_t<decltype(_CUDA_VSTD::move(fn)(_CUDA_VSTD::forward<_Args>(args)...))>;
    constexpr auto __bytes = __aligned_size(sizeof(_Ty));
    auto* pb               = __buffer_ + (__descriptors_[__token].__offset - __bytes);
    auto* __ptr            = ::new (pb) _Ty(_CUDA_VSTD::move(fn)(_CUDA_VSTD::forward<_Args>(args)...));
  ...
  {
    constexpr auto __bytes = __aligned_size(sizeof(_Ty));
    auto __offset          = __descriptors_[__token].__offset - __bytes;
    return *reinterpret_cast<_Ty*>(__buffer_ + __offset);
  }

  using __descriptor_t                  = __storage_descriptor;
  const __descriptor_t* __descriptors_  = nullptr; // points to managed memory
...
struct __storage_registry_context
{
  ...
  _CUDAX_HOST_API ~__storage_registry_context() noexcept
  {
    for (auto& __desc : __host_descriptors_)
    {
      if (__desc.__destroy)
      {
        __desc.__destroy(__buffer_.data() + __desc.__offset);
      }
    }
  }
  ...
  _CUDAX_HOST_API auto __finalize() -> __storage_registry
  {
    ...
    __buffer_ = __storage_registry_buffer_t{__mr, __buffer_.get_stream(), __host_descriptors_.back().__offset};

    // Copy the __descriptors_ into the __buffer_:
    auto* __device_descriptors =
      reinterpret_cast<__descriptor_t*>(__buffer_.data() + (__host_descriptors_[__token].__offset - __bytes));
...
private:
  ...
  _CUDAX_HOST_API auto __reserve(size_t __bytes) -> size_t
  {
    ...
    auto __base  = __token ? __host_descriptors_.back().__offset : 0;
