...
{
  _CUDAX_SEMI_PRIVATE :
  using __call_result_or_void =
    ::cuda::std::__type_try_catch<::cuda::std::__type_quote<__call_result_t>, ::cuda::std::__type_always<void>>;
  ...
  template <class _Sndr, class _NewSch, class _OldSch, class... _Closure>
  __lowered_sndr_t(_Sndr, _NewSch, _OldSch, _Closure...) -> __lowered_sndr_t<_Sndr, _NewSch, _OldSch, _Closure...>;
...
public:
  ...
  [[nodiscard]] _CCCL_API static constexpr auto transform_env(const _Sndr& __sndr, _Env&& __env)
  {
    ...
    {
      return __join_env(__sch_env_t{__data}, static_cast<_Env&&>(__env));
    }
    else
    {
  ...
  [[nodiscard]] _CCCL_API static constexpr auto transform_sender(_Sndr&& __sndr, const _Env& __env)
  {
    ...
    if constexpr (__is_scheduler<decltype(__data)>)
    {
      auto __old_sch = __call_or(get_scheduler, __not_a_scheduler<_Env>{}, __env);
      return __lowered_sndr_t{::cuda::std::forward_like<_Sndr>(__child), __data, __old_sch};
    }
    else
    {
      auto& [__new_sch, __closure] = __data;
      auto __old_sch =
        __call_or(get_completion_scheduler<set_value_t>, __not_a_scheduler<_Env>{}, execution::get_env(__child), __env);
      return __lowered_sndr_t{
...
{
private:
  template <class _SetTag, class _Env, class _OldSch = __call_result_t<get_scheduler_t, _Env&>>
    ...
    _SetTag, [[maybe_unused]] const __sndr_t& __self, [[maybe_unused]] _Env&& __env) noexcept
  {
    ...
    else
    {
      ...
      constexpr bool __sndr_has_compl    = __has_completions_for<_SetTag, _Sndr, __join_env_t<__sch_env_t<_Sch>, _Env>>;
        ...
        if constexpr (__new_sch_has_compl && __callable<execution::get_completion_scheduler_t<_SetTag>, _Sch, _Env&>)
        {
          return execution::get_completion_scheduler<_SetTag>(__self.__sch_, __env);
                           ...
                           && __callable<execution::get_completion_scheduler_t<_SetTag>,
                                         env_of_t<_Sndr>,
                                         __join_env_t<__sch_env_t<_Sch>, _Env>>)
        {
          return execution::get_completion_scheduler<_SetTag>(
            execution::get_env(__self.__sndr_), __join_env(__sch_env_t{__self.__sch_}, static_cast<_Env&&>(__env)));
        }
  ...
  __get_completion_domain_for(_SetTag, [[maybe_unused]] const __sndr_t& __self, [[maybe_unused]] _Env&& __env) noexcept
  {
    ...
    else
    {
      ...
      using __sndr_domain_t =
        __type_call_or_q<__call_result_t,
                         void,
  ...
  struct __attrs_t
  {
    ...
    {
      using __old_sch_t = __call_result_or_t<get_scheduler_t, __not_a_scheduler<_Env>, _Env>;
      return (execution::min) (execution::get_completion_behavior<schedule_result_t<_Sch>, _Env>(),
                               execution::get_completion_behavior<_Sndr, __join_env_t<__sch_env_t<_Sch>, _Env>>(),
...
public:
  ...
  _CCCL_TRIVIAL_API constexpr auto get_env() const noexcept -> __attrs_t
  {
    return {__sndr_};
  }
