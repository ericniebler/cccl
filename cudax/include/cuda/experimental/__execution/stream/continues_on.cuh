...
struct stream_domain::__apply_t<continues_on_t>
{
  ...
  // delegation scheduler (the run_loop within sync_wait). That thunk will then
  // schedule the continuation on the destination scheduler.
  template <class _DestinationSched, class _DelegationSched, class _Rcvr>
  struct __thunk_opstate_t
  {
    using __run_rcvr_t    = __rcvr_ref<__thunk_opstate_t, env_of_t<_Rcvr>>;
    using __run_opstate_t = connect_result_t<schedule_result_t<_DelegationSched>, __run_rcvr_t>;
    using __dst_opstate_t = connect_result_t<schedule_result_t<_DestinationSched>, __rcvr_ref<_Rcvr>>;

    _CCCL_API explicit __thunk_opstate_t(_DestinationSched __dst_sched, _DelegationSched __run_sched, _Rcvr __rcvr)
        : __rcvr_(static_cast<_Rcvr&&>(__rcvr))
        , __dst_sched_(static_cast<_DestinationSched&&>(__dst_sched))
        , __opstate_()
    {
      __opstate_.__emplace_from(connect, schedule(__run_sched), __rcvr_ref{this});
    }

    _CCCL_API void start() noexcept
    {
      __opstate_.template __get<0>().start();
    }
    ...
    _CCCL_API void set_value() noexcept
    {
      _CUDAX_TRY( //
        ({ //
          auto& __opstate =
            __opstate_.__emplace_from(connect, schedule(__dst_sched_), __rcvr_ref{_CUDA_VSTD::addressof(__rcvr_)});
          __opstate.start();
        }),
        _CUDAX_CATCH(...) //
        ({ //
  ...
  struct __thunk_sndr_t
  {
    ...
    template <class _Rcvr>
    _CCCL_API auto connect(_Rcvr __rcvr) const -> __opstate_t<_Rcvr>
    {
      return __opstate_t<_Rcvr>{__dst_sched_, __run_sched_, static_cast<_Rcvr&&>(__rcvr)};
    }

    _CCCL_API auto get_env() const noexcept -> prop<get_completion_scheduler_t<set_value_t>, const _DestinationSched&>
    {
      return {get_completion_scheduler<set_value_t>, __dst_sched_};
    }
  ...
  template <class _DestinationSched, class _DelegationSched>
  struct __thunk_sched_t
  {
    using scheduler_concept = scheduler_t;

    _CCCL_API auto schedule() const noexcept
    {
      return __thunk_sndr_t<_DestinationSched, _DelegationSched>{__dst_sched_, __run_sched_};
    }
    ...
    _CCCL_API friend auto operator==(const __thunk_sched_t& __lhs, const __thunk_sched_t& __rhs) noexcept -> bool
    {
  ...
  template <class _DestinationSched, class _DelegationSched>
  _CCCL_HOST_DEVICE __thunk_sched_t(_DestinationSched, _DelegationSched)
    -> __thunk_sched_t<_DestinationSched, _DelegationSched>;

  template <class _Sndr, class _Env>
  _CCCL_API auto operator()(_Sndr&& __sndr, const _Env& __env) const -> decltype(auto)
  {
    auto&& [__tag, __sched, __child] = static_cast<_Sndr&&>(__sndr);
    auto __run_sched                 = get_delegation_scheduler(__env);
    return execution::schedule_from(__thunk_sched_t{__sched, __run_sched}, _CUDA_VSTD::forward_like<_Sndr>(__child));
  }
};
} // namespace cuda::experimental::execution
