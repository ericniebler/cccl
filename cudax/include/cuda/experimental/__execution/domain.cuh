...
{
template <class _Env>
struct __stop_recursing
{
  _CCCL_TEMPLATE(class _Query, class... _Args)
  _CCCL_REQUIRES(__queryable_with<_Env, _Query, _Args...>)
  _CCCL_API auto query(_Query, _Args&&...) const noexcept(__nothrow_queryable_with<_Env, _Query, _Args...>)
    -> __query_result_t<_Env, _Query, _Args...>;
};

template <class _Sch>
_CCCL_API auto __get_scheduler_domain()
  -> __call_result_t<get_completion_domain_t<set_value_t>, __stop_recursing<env_of_t<schedule_result_t<_Sch>>>>;

template <class _Sch, class _Env>
_CCCL_API auto __get_scheduler_domain()
  -> __call_result_or_t<get_completion_domain_t<set_value_t>,
                        default_domain,
                        __stop_recursing<env_of_t<schedule_result_t<_Sch>>>,
                        _Env>;
} // namespace __detail
...
struct get_domain_t
{
  ...
  [[nodiscard]] _CCCL_TRIVIAL_API constexpr auto operator()(const _Env&) const noexcept
  {
    using __domain_t = __scheduler_domain_t<__call_result_t<get_scheduler_t, const _Env&>, const _Env&>;
    static_assert(__domain_like<__domain_t>, "Domain types are required to be empty class types");
    return __domain_t{};
  }
...
struct get_completion_domain_t
{
  ...
  //! @brief If there is a \c get_completion_domain_t attribute in \c __attrs,
  //! return it.
  _CCCL_EXEC_CHECK_DISABLE
  _CCCL_TEMPLATE(class _Attrs)
  _CCCL_REQUIRES(__queryable_with<_Attrs, get_completion_domain_t>)
  [[nodiscard]] _CCCL_TRIVIAL_API constexpr auto operator()(const _Attrs&, ::cuda::std::__ignore_t = {}) const noexcept
  {
  ...
  _CCCL_TEMPLATE(class _Attrs,
                 class... _Env,
                 ::cuda::std::enable_if_t<!__is_specialization_of_v<_Attrs, __detail::__stop_recursing>, int> = 0)
  _CCCL_REQUIRES((!__queryable_with<_Attrs, get_completion_domain_t>) _CCCL_AND //
                 (!__queryable_with<_Attrs, get_completion_domain_t, const _Env&...>) _CCCL_AND //
                   __callable<get_completion_scheduler_t<_Tag>, const _Attrs&, const _Env&...>)
  [[nodiscard]] _CCCL_TRIVIAL_API constexpr auto operator()(const _Attrs& __attrs, const _Env&...) const noexcept
    -> __scheduler_domain_t<__call_result_t<get_completion_scheduler_t<_Tag>, const _Attrs&, const _Env&...>,
                            const _Env&...>
  {
    using __scheduler_t = __call_result_t<get_completion_scheduler_t<_Tag>, const _Attrs&, const _Env&...>;
    using __domain_t    = __scheduler_domain_t<__scheduler_t, _Env...>;
    static_assert(__domain_like<__domain_t>, "Domain types are required to be empty class types");
    return __domain_t{};
  }

  //! @brief If \c __attrs has neither a completion domain nor a completion scheduler, but
  //! the sender is known to complete inline and \c __env has a domain, then return that
  //! domain.
  _CCCL_EXEC_CHECK_DISABLE
  _CCCL_TEMPLATE(class _Attrs, class _Env)
  _CCCL_REQUIRES((!__queryable_with<_Attrs, get_completion_domain_t>) _CCCL_AND //
                 (!__queryable_with<_Attrs, get_completion_domain_t, const _Env&>) _CCCL_AND //
                 (!__callable<get_completion_scheduler_t<_Tag>, const _Attrs&, const _Env&>) _CCCL_AND //
                   __completes_inline<_Attrs, _Env> _CCCL_AND __callable<get_domain_t, const _Env&>)
  [[nodiscard]] _CCCL_TRIVIAL_API constexpr auto operator()(const _Attrs&, const _Env&) const noexcept
  {
