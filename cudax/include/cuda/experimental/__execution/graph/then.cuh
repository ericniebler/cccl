...
{
template <>
struct _CCCL_TYPE_VISIBILITY_DEFAULT graph_domain::__apply_t<then_t>
{
  ...
  _CCCL_HOST_API auto operator()(_Context& __context, _Fn&& __fn, _Sndr&& __sndr) const -> __visit_result_t
  {
    ...
    // Release ownership of the predecessor so it is not deleted when this function
    // returns.
    __pred_result.__node_.release();
