  ...
  struct __attrs_t
  {
    ...
    _CCCL_API static constexpr auto __get_domain_override() noexcept
    {
      if constexpr (__same_as<_Tag, schedule_from_t>)
      {
        ...
        return __type_call_or<::cuda::std::__type_indirect_quote<__scheduler_domain_t>,
                              void,
                              _Sch,
                              __fwd_env_t<_Env>...>();
      }
      else if constexpr (__callable<get_completion_domain_t<set_value_t>, env_of_t<_Sndr>, __fwd_env_t<_Env>...>)
      {
        // for continues_on, return the domain of the predecessor sender:
        return __call_result_t<get_completion_domain_t<set_value_t>, env_of_t<_Sndr>, __fwd_env_t<_Env>...>();
      }
    }

    template <class... _Env>
    using __domain_override_t _CCCL_NODEBUG_ALIAS =
      __unless_one_of_t<decltype(__attrs_t::__get_domain_override<_Env...>()), void>;
