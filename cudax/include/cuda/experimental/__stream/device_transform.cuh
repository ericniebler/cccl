...
// 4. `arg`
_CCCL_HOST_API void cuda_device_transform();
...
struct _CCCL_TYPE_VISIBILITY_DEFAULT __device_transform_t
{
  ...
  struct _CCCL_TYPE_VISIBILITY_DEFAULT __storage_for
  {
    _CCCL_HOST_API constexpr __storage_for() noexcept {}

    _CCCL_HOST_API ~__storage_for()
    {
      if (__engaged)
      {
        _CUDA_VSTD::destroy_at(_CUDA_VSTD::addressof(__value));
      }
    }

    template <class... _OtherArgs>
    [[nodiscard]] _CCCL_HOST_API auto
    __emplace(_OtherArgs&&... __arg) noexcept(__nothrow_constructible<_Arg, _OtherArgs...>) -> _Arg&&
    {
      _Arg* __ptr =
        _CUDA_VSTD::__construct_at(_CUDA_VSTD::addressof(__value), _CUDA_VSTD::forward<_OtherArgs>(__arg)...);
      __engaged = true;
  ...
  // _Arg is a reference type in this specialization. There is no need to store reference
  // types since they are not ephemeral.
  template <class _Arg>
  struct __storage_for<_Arg, true>
  {
    [[nodiscard]] _CCCL_HOST_API auto __emplace(_Arg __arg) noexcept -> _Arg
    {
  ...
  //   auto operator()(::cuda::stream_ref stream, Arg&& arg, auto&& action = cuda_device_transform(arg))
  //
  // but sadly that is not valid C++.
  template <typename _Arg>
  [[nodiscard]] _CCCL_HOST_API auto
  operator()(::cuda::stream_ref __stream, _Arg&& __arg, __storage_for<__transform_result_t<_Arg>> __storage = {}) const
    -> decltype(auto)
  {
    ...
    {
      return __storage
        .__emplace(__emplace_from{[&]() -> decltype(auto) {
    ...
    {
      return __storage.__emplace(__emplace_from{[&]() -> decltype(auto) {
        return cuda_device_transform(__stream, _CUDA_VSTD::forward<_Arg>(__arg));
      }});
    }
  }
