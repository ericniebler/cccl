...
{
  auto snd = ex::starts_on(inline_scheduler<>{}, ex::just(42));
...
C2H_TEST("starts_on forwards sender errors", "[adaptors][starts_on]")
{
  ...
  auto snd = ex::starts_on(inline_scheduler<>{}, ex::just_error(ec));
  auto op  = ex::connect(std::move(snd), checked_error_receiver{ec});
  ex::start(op);
}
...
{
  auto snd = ex::starts_on(inline_scheduler<>{}, ex::just_stopped());
  auto op  = ex::connect(std::move(snd), checked_stopped_receiver{});
  ex::start(op);
}
...
{
  auto snd = ex::starts_on(inline_scheduler<>{}, ex::just(1, 2.5, string{"hello"}));
...
C2H_TEST("starts_on works with const sender", "[adaptors][starts_on]")
{
  ...
  auto snd               = ex::starts_on(inline_scheduler<>{}, base_sender);
  auto op                = ex::connect(std::move(snd), checked_value_receiver{42});
  ex::start(op);
}
