...
struct _impulse_scheduler_data_t : std::enable_shared_from_this<_impulse_scheduler_data_t>
{
  explicit _impulse_scheduler_data_t(int id)
      : id_(id)
  {}
...
struct _impulse_scheduler_attrs_t
{
  _impulse_scheduler_data_t* data_;

  template <class Env>
  auto query(ex::get_completion_scheduler_t<ex::set_value_t>, const Env&) const noexcept
    -> cuda::std::__call_result_t<ex::get_scheduler_t, const Env&>;

  template <class Env>
  auto query(ex::get_completion_scheduler_t<ex::set_stopped_t>, const Env&) const noexcept
    -> cuda::std::__call_result_t<ex::get_scheduler_t, const Env&>;
...
struct impulse_scheduler : _impulse_scheduler_attrs_t
{
private:
  ...
  using _data_t    = _impulse_scheduler_data_t;
  friend struct _impulse_scheduler_attrs_t;
  ...
  struct _opstate_t : cuda::__immovable
  {
    ...
    _opstate_t(_data_t* data, Rcvr&& rcvr)
        : _data_(data)
  ...
  struct _sndr_t
  {
    ...
    {
      return {_data_, static_cast<Rcvr&&>(rcvr)};
    }

    auto get_env() const noexcept
    {
      return _impulse_scheduler_attrs_t{_data_};
    }
  };
...
template <class Env>
auto _impulse_scheduler_attrs_t::query(ex::get_completion_scheduler_t<ex::set_value_t>, const Env& __env) const noexcept
  -> cuda::std::__call_result_t<ex::get_scheduler_t, const Env&>
{
...
template <class Env>
auto _impulse_scheduler_attrs_t::query(ex::get_completion_scheduler_t<ex::set_stopped_t>,
                                       const Env& __env) const noexcept
  -> cuda::std::__call_result_t<ex::get_scheduler_t, const Env&>
{
  return get_scheduler(__env);
}
